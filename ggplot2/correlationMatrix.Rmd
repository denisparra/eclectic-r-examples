...
Correlation Analysis of the seller buyer data from the Second Life (SL) dataset

```{r}
setwd("C:/Users/Denis/Dropbox/HT2014/3000_no_threshold");
sellbuy_SL <- read.csv(file="02_feed_seller_buyer_3000_no_threshold.csv", header = TRUE, sep = "\t");
```

Describe the dataset
-------------------------

# describe the sellbuy_SL dataframe
...
Mcor <- cor(sellbuy_SL[,1:26]);

```

Set the Cairo device for better plotting
```{r}
library(Cairo)
```
...
A) features are ordered alphabetically
...
B) features are ordered  angular order of the eigenvectors
...
```{r echo=FALSE}
#re-declare function
# Redefinition of function to fit different text size
corrplotd <- function (corr, method = c("circle", "square", "ellipse", "number", 
    ...
    uppCI.mat = NULL, corr.cex = 1, ...) 
{
    ...
    if (method == "number" & plotCI == "n") {
        text(Pos[, 1], Pos[, 2], font = 2, cex= corr.cex, col = col.fill, labels = round((DAT - 
...
C) Numeric 
```{r dev='CairoPNG'}
corrplotd(Mcor, method="number", tl.pos = "lt", tl.cex=0.7, tl.srt= 60, cl.cex = 0.6,  type="lower", title="Correlation Matriz, undirected Seller-Buyer network", diag = FALSE, corr.cex=0.5)
...
```{r}
# Redefinition of function to fit different text size
corrplotd.mixed <- function (corr, lower = "number", upper = "circle", tl.pos = c("d", 
    "lt", "n"), diag = c("n", "l", "u"), bg = "white", addgrid.col = "gray", corr.cex = 1,
    ...) 
{
    diag <- match.arg(diag)
    ...
    corrplot(corr, type = "upper", method = upper, diag = TRUE, 
        tl.pos = tl.pos, ...)
    corrplotd(corr, add = TRUE, type = "lower", method = lower, 
        diag = (diag == "l"), tl.pos = "n", cl.pos = "n", corr.cex = corr.cex , ...)
    if (diag == "n" & tl.pos != "d") {
...
D) Mixed (numeric and circles)
```{r fig.width=11, fig.height=9, dev='CairoPNG'}
corrplotd.mixed(Mcor, tl.pos = "lt", tl.cex=0.8, tl.srt= 45, cl.cex = 0.7, title="Correlation Matriz, undirected Seller-Buyer network", corr.cex=0.7)
```
